...
function Win:UpdateFields()
	...
	self.BtnTargeting.caption = "自动控制 (Cr. " .. toReadableValue(Config.CostTargeting) .. ")"
	self.BtnCoaxial.caption = "同轴武器 (Cr. " .. toReadableValue(Config.CostCoaxial) .. ")"
	self.BtnColour.caption = "武器颜色 (Cr. " .. toReadableValue(Config.CostColour) .. ")"
	self.BtnSize.caption = "规模 (Cr. " .. toReadableValue(Config.CostSize) .. ")"

	self.BtnHeat.caption = "散热器"
	self.LblHeat.caption = HeatRate .. " 热量, " .. CoolRate .. " 冷却"
	self.LblHeat.color = ColourLight

	self.BtnBaseEnergy.caption = "电容器"
	self.LblBaseEnergy.caption = BaseEnergy .. " 功率要求"
	self.LblBaseEnergy.color = ColourLight

	self.BtnAccumEnergy.caption = "变压器"
	self.LblAccumEnergy.caption = AccumEnergy .. " 能量要求"
	self.LblAccumEnergy.color = ColourLight

	self.BtnDamage.caption = "火力"
	self.LblDamage.caption = Damage .. " (" .. round((Damage * FireRate * GunCount),2) .. " DPS)"
	self.LblDamage.color = ColourLight

	self.BtnFireRate.caption = "炮塔射速"
	self.LblFireRate.caption = FireRate .. " RPS"
	self.LblFireRate.color = ColourLight

	self.BtnSpeed.caption = "炮塔转速"
	self.LblSpeed.caption = Speed
	self.LblSpeed.color = ColourLight

	self.BtnRange.caption = "射程"
	self.LblRange.caption = Range .. " KM"
	self.LblRange.color = ColourLight

	self.BtnAccuracy.caption = "精度"
	self.LblAccuracy.caption = (Accuracy * 100) .. "%"
	self.LblAccuracy.color = ColourLight

	self.BtnEfficiency.caption = "效率"
